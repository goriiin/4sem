...
# Генерация данных для таблицы cheque
for i in range(1000, 1000001):  # количество строк для генерации
    rent_id = fake.random_int(min=1, max=999)  # случайный идентификатор аренды
    user_id = fake.random_int(min=1, max=999)  # случайный идентификатор пользователя
    _sum = fake.random_int(100, 1000) # случайная общая стоимость
    payment_status = fake.boolean()  # случайный статус оплаты
